...
#let VSTROKE = 1pt + luma(200)

#let END_MARKER = align(horizon, pad(left: 3pt, box(line(length: 14pt, stroke: VSTROKE))) + place(rotate(90deg, line(length: 6pt, stroke: VSTROKE)), dy: -3pt, dx: 0.3pt))
...
#let algorithm(inset: 0.2em, indent: 0.5em, vstroke: 0pt + luma(200), style: it => it, ..bits) = {
  ...
  let indent-content = indent-list.enumerate().map(((k,i)) => ([], grid.vline(stroke: vstroke), []).filter(i => i != none) * int(i / 2))
  ...
  while line-number <= content.len() {
    grid-bits.push(style[#set text(0.8em);#v(0.2em)#line-number])
...
#let call(name, kw: "function", inline: false, style: smallcaps, args, ..body) = (
  if inline {
    ...
    iflike(kw1: kw, kw3: END_MARKER, (style(name) + $(#arraify(args).join(", "))$), ..body)
  }
)
...
#let LineComment(l, c) = (grid(columns: (40%, auto), l.first(), CommentInline(c)), ..l.slice(1),)

// Control flow
#let If = iflike.with(kw1: "if", kw2: "then", kw3: END_MARKER)
#let While = iflike.with(kw1: "while", kw2: "do", kw3: END_MARKER)
#let For = iflike.with(kw1: "for", kw2: "do", kw3: END_MARKER)
#let Else = iflike.with(kw1: "else", kw2: "", kw3: END_MARKER, "")
#let ElseIf = iflike.with(kw1: "else if", kw2: "then", kw3: END_MARKER)
  ...
  while i < len {
    if i == len - 1 and calc.odd(len) {
      ...
      if i == 0 {
        ...
        // Last condition before "else" is an "elseif" with END_MARKER
        result.push(ElseIf(cond, ..body, kw3: END_MARKER))
      } else {
        // Intermediate conditions are "elseif" without END_MARKER
